...
- Basal insulin uses a Bateman one-compartment PK curve (`basal_curve`): `f(t) = exp(-ke·t) − exp(-ka·t)`, with `BASAL_KA_PER_HOUR = 0.6` and `BASAL_KE_PER_HOUR = 0.09` giving a peak at ~3.7h post-injection and a long ~7.7h-half-life decline over `BASAL_DURATION_HOURS` (28h). This replaces the earlier trapezoid — real long-acting basal has no flat plateau, so the new shape has none either. A smootherstep window over the final `BASAL_TAIL_CLIP_HOURS` tapers the late residual to zero so consecutive daily doses join without a tail-step. The legacy `BASAL_RAMP_UP_HOURS` / `BASAL_RAMP_DOWN_HOURS` constants are retained as backward-compat aliases (now interpretable as "approx time-to-peak" and "tail-clip duration") used only by tests for warmup math.

...
> **Caveat.** T1DMSIM was calibrated against the Ohio/Shanghai cohorts;
...
> ({cfg['uva_dosing']}). The calibration targeted
> both **level** and distributional *shape* (the ΔBG distribution and
> autocorrelation), so the **dynamic** metrics are not a fitting-free contest
> either — merely less dominated by the level fit. Every gap below is a distance
> from a target T1DMSIM was trained toward and UVA/Padova never saw.
...
On the marginal BG distribution **{closer}** is closer to real, by ≈{ratio:.1f}× on
Wasserstein.
...
(mean normalised error {v['level_norm_err']['T1DMSIM']:.2f} vs {v['level_norm_err']['UVA/Padova']:.2f} real-patient SDs) and on
dynamics ({v['dyn_norm_err']['T1DMSIM']:.2f} vs {v['dyn_norm_err']['UVA/Padova']:.2f}). Its pooled BG-distance to real
({wb['T1DMSIM']:.0f} mg/dL) falls **below** the real-vs-real floor ({wb['floor']:.0f} mg/dL) — inside
the spread among the real cohorts themselves — whereas UVA/Padova
({wb['UVA/Padova']:.0f} mg/dL) sits outside it. The ordering also holds on the
excursion-amplitude metrics (MAGE, MODD). None of this is a transfer guarantee:
T1DMSIM was fit to these cohorts and UVA/Padova never saw them, so the result
confirms the calibration rather than crowning a winner.

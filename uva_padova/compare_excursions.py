...
{c['days']}-day schedule. The food-meal times and grams it produces are shared
with a paired UVA/Padova `{c['uva_group']}` patient; insulin is then **not**
...
for its own physiology — {c['uva_dosing']}. With both engines in a normal
operating range, isolated post-meal excursions (≥{c['meal_min_g']:.0f} g, no
other ≥{ISO_OTHER_G:.0f} g meal within ±{ISO_PRE_MIN} min so the rise and peak
are clean, a {c['window_min']}-min window) are extracted
relative to each engine's **own** pre-meal baseline, so what is compared is the
*response*, not the level. {c['n_excursions']} excursions met the criteria.

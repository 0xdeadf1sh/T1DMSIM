...
def _render_dataset_md(report: dict[str, Any]) -> str:
    ...
    lines.append(
        ...
        f'patient trajectories, each **{c["keep_hours"]:.1f} h** of post-warmup CGM '
        f'at {int(c["dt_minutes"])}-minute cadence, stored as one compressed '
        f'`.b2nd` file per channel.'
    )
    lines.append('')
    lines.append(
        'Generated by `cache_simulator.py` during the build; figures cover every '
        'cached trajectory.'
    )
    lines.append('')
    ...
    lines.append('')
    lines.append(
        'Fraction of total CGM *time* in each glycemic class; the bands partition '
        'the pooled readings and sum to 100%.'
    )
    lines.append('')
    ...
    lines.append('')
    lines.append(
        'The carb-appearance channel folds meal, hypo-rescue, and follow-up '
        'carbohydrate together, so the event count (upward threshold-crossings > '
        f'{MEAL_CARB_THRESHOLD_G:.0f} g/step, {EVENT_REFRACTORY_MIN:.0f}-minute '
        'refractory) is an upper bound on discrete meals. The bolus count merges '
        'boluses whose pharmacokinetic tails overlap and is therefore a lower bound.'
    )
    lines.append('')

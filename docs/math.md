...
A low that is not ending is treated with a rage-eat instead of another nibble: `uniform(RAGE_EAT_CARB_MIN, RAGE_EAT_CARB_MAX)` grams of fast carbs with no recheck and no follow-up, clearing the rescue history. It fires on the `HYPO_STUCK_RESCUES`-th rescue inside `HYPO_STUCK_WINDOW_MIN`; on any rescue attempt still below the threshold `HYPO_STUCK_STILL_LOW_MIN` after the previous one; on the second rescue of a severe (< 55) episode when `RAGE_EAT_ON_SECOND_SEVERE`; and on every rescue while the reading is below `RAGE_EAT_BG_THRESHOLD`. The refractory is waived below `RAGE_EAT_BG_THRESHOLD` and in the still-low case. Independently, each rescue rage-eats with probability `RAGE_EAT_PROBABILITY_BASE * (1 - dosing_competence)`.
